...
public class GameMap {
    ...
    private TextureRegion grassAnyTexture;
    ...
    public GameMap() {
        ...
        int grassAnyCount = (int) ((CELLS_X * CELLS_Y * FOREST_PERCENTAGE) / 100.0f);
        for (int i = 0; i < grassAnyCount; i++) {
            this.data[MathUtils.random(0, CELLS_X - 1)][MathUtils.random(0, CELLS_Y - 1)].changeType(CellType.GRASSANY);
        ...
        this.grassAnyTexture = Assets.getInstance().getAtlas().findRegion("grassAny");
    }
    ...
    public void render(SpriteBatch batch) {
        for (int i = 0; i < CELLS_X; i++) {
            for (int j = CELLS_Y - 1; j >= 0; j--) {
                ...
                if (data[i][j].type ==CellType.GRASSANY) {
                    batch.draw(grassAnyTexture, i * CELL_SIZE, j * CELL_SIZE);
                }
            }
        }
    }
    ...
    public void checkAndTakeDrop(Unit unit) {
        ...
        if (currentCell.type == CellType.GRASSANY) {
            return;
        }
